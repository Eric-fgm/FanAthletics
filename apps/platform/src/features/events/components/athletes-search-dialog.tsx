import type { AthleteWithDisciplines, Discipline } from "@fan-athletics/shared/types";
import { CircleUser } from "lucide-react-native";
import type React from "react";
import { useState } from "react";
import { Pressable, ScrollView, TextInput, View, Image } from "react-native";
import { Button, Dialog, Dropdown, Switch, Toggle, Typography } from "#/components";
import {
...
const AthletesSearchDialog: React.FC<AthletesSearchDialogProps> = ({
	...
	const nationalities = [...new Set(athletes.map((athlete) => athlete.nationality))];
	...
	const filteredAthletes = athletes.filter((athlete) => {
		...
		const startsInCurrentDiscipline = currentDiscipline === undefined
			|| athlete.disciplines.some((discipline) => discipline.id === currentDiscipline.id);
		const costBetweenMinAndMax = athlete.cost >= minAthleteCost && athlete.cost <= maxAthleteCost;
		const affordableInBudget = inBudget && athlete.cost <= budget || !inBudget;
		const nationalityMatches = currentNationality === "" || athlete.nationality === currentNationality;
	...
	return (
		<Dialog {...props}>
			<View className="px-5 py-4 h-full" style={{ flexGrow: 1, minHeight: 524 }}>
				<Typography size="large" className="mb-3">Dodaj zawodnika</Typography>
				<View className="flex-col md:flex-row">
					<View className="flex-col rounded-2xl p-3 flex-[0.4] h-full" style={{ backgroundColor: "#FFC5C5" }}>
						<TextInput
						...
						/>
						<View className="w-fullh-[10px] rounded-2xl mb-3" style={{ height: 2, backgroundColor: "#973232" }}/>
						<Switch
							items={[
						...
						/>
						<Typography size="large1" className="mb-2 px-2">Konkurencja</Typography>
						<Dropdown
							trigger={<Button
									text={currentDiscipline === undefined ?
										"Wybierz dyscyplinę" : currentDiscipline.name
									}
									className="w-full"
									rounded
								/>
						...
						/>
						<Typography size="large1" className="mt-4 mb-2 px-2">Koszt</Typography>
						<View className="flex-row w-full gap-2 mb-2">
							<TextInput
								...
								onChange={(event) => setMinAthleteCost(Math.min(Number(event.nativeEvent.text) || 0, MAX_ATHLETE_COST))}
							/>
							<TextInput
								placeholder="Max"
								keyboardType="numeric"
								...
								onChange={(event) => setMaxAthleteCost(Math.min(Number(event.nativeEvent.text) || MAX_ATHLETE_COST, MAX_ATHLETE_COST))}
							/>
						</View>
						<View className="flex-row w-full mb-4">
							<Typography size="large1" className="px-2">W budżecie</Typography>
							<Toggle value={inBudget} onChangeValue={() => setInBudget(!inBudget)} className="ms-auto"/>
						</View>
						<Typography size="large1" className="px-2 mb-2">Narodowość</Typography>
						<Dropdown
							trigger={<Button text={currentNationality !== "" ?
									currentNationality :
									"Wybierz narodowość"} 
								/>
							}
